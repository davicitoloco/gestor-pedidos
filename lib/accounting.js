...
function recordJournal({ date, desc, ref_type, ref_id, lines, userId }) {
  ...
  const { lastInsertRowid } = db.prepare(
    'INSERT INTO journal_entries (date, description, ref_type, ref_id, created_by) VALUES (?,?,?,?,?)'
  ).run(date || new Date().toISOString().slice(0,10), desc, ref_type||'', ref_id||null, userId||null);
  const entryId = Number(lastInsertRowid);
  const ins = db.prepare('INSERT INTO journal_entry_lines (entry_id, account_id, debit, credit) VALUES (?,?,?,?)');
  for (const l of lines) ins.run(entryId, l.account_id, l.debit||0, l.credit||0);
  return entryId;
}
